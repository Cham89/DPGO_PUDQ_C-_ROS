...
namespace opt{
    ...
    std::tuple<double, double> Optimization_RLM_inverse_edge(pudqlib::LocalGraph& local_graph, double mu_in, double grad_tol) {
        ...
        const double grad_norm = pudqlib::riemannian_norm(local_graph, rgrad, offset);
        ...
        for (int lm_trial = 0; lm_trial < max_lm_trials; ++lm_trial) {
            ...
            // g_k^T * G * s_k
            double linear_term = pudqlib::riemannian_dot(local_graph, g_k, s_k_trunc, offset);
            // s_k^T * G * (H * s_k)
            double quad_term = 0.5 * pudqlib::riemannian_dot(local_graph, s_k_trunc, H_k_reg * s_k_trunc, offset);
            const double pred_reduction = -(linear_term + quad_term);
            ...
            if (rho > eta) {
                ...
                const double norm_grad_F = pudqlib::riemannian_norm(local_graph, result_final.rgrad_X, offset);
    ...
    void check_all_robotgrad_inverse_edge(std::vector<pudqlib::Robot>& robotList, double grad_tol){
        ...
        for (std::size_t r = 0; r < num_robots; ++r){
            ...
            const double norm_grad_F = pudqlib::riemannian_norm(local_graph_r, result.rgrad_X, offset);
