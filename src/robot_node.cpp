...
public:
    ...
    // ======================================================================
    // THREAD 2: Communication Send (Periodic)
    // ======================================================================
    ...
    // ======================================================================
    // THREAD 3a: Communication Receive (Data)
    // ======================================================================
    void communicationCallback(const distributed_pgo::PgoUpdate::ConstPtr& msg) {
        ...
        // Scope 1: Lock Graph, Update, and Calculate Gradient
        {
            std::lock_guard<std::mutex> lock(m_graph_mutex);
            ...
            // Immediate Gradient Check (While holding graph lock)
            if (graph_updated) {
        ...
        // Scope 2: Update Status & Check Global (No Graph Lock needed)
        if (graph_updated) {
    ...
    // ======================================================================
    // THREAD 3b: Status Receive (FIXED DEADLOCK)
    // ======================================================================
    void statusCallback(const distributed_pgo::PgoStatus::ConstPtr& msg) {
        if (msg->sender_robot_id == m_robot_id) return;

        // 1. Update Table (Lock Scope restricted to this block)
        {
            std::lock_guard<std::mutex> lock(m_status_mutex);
        ...
        // 2. Check Global Convergence (Re-acquires lock safely)
        checkGlobalConvergence();
    }

    // ======================================================================
    // POST-PROCESSING: Save & Print
    // ======================================================================
    void saveFinalResults() {
        // 1. Calculate Final Local Stats
        ...
        // 2. Print to Console (Matches your snippet's style)
        std::printf("\n*** Robot %d Finished! ***\n", m_robot_id);
        std::printf("Final Local Cost=%.6g, Final Local Grad=%.6g\n", final_cost, final_grad);

        // 3. Export Local Graph to File
        // We cannot use 'graph::saveFullAnalysisData' because that requires ALL robots.
        // We save ONLY our local part.
        try {
            ...
            // Use the helper function from graph.hpp (assuming it's accessible/templated)
...
private:
    ...
    void checkGlobalConvergence() {
        bool all_true = true;
        {
            std::lock_guard<std::mutex> lock(m_status_mutex); // Safe lock acquisition
