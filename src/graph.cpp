...
namespace graph{
    ...
    pudqlib::Graph reconstructglobalgraph(const std::vector<pudqlib::Robot>& robots, const pudqlib::Graph& original_G
        ...
        for (int r = 0; r < num_robots; ++r) {
            ...
            // Optional: Logging to match MATLAB output
            ...
            for (int local_idx = 0; local_idx < num_local_vertices; ++local_idx) {
                ...
                // Check if local_idx is valid for the mapping vector
                ...
                // Check if the global index is valid for the global graph
                if (global_idx >= 0 && global_idx < total_vertices) {
                    
                    // Get the optimized local PUDQ pose
                    const pudqlib::PUDQ& optimized_pudq = local_graph.vertices_pudq[local_idx];

                    // Update the global graph's PUDQ vector
                    G_optimized.vertices_pudq[global_idx] = optimized_pudq;

                    // Update the global graph's pose (Vector3d) vector
    ...
    Eigen::MatrixXd readMatrixFromFile(const std::string& filename) {
        ...
        if (v.empty()) {
             // Return an empty matrix if the file was empty or only had comments
        ...
        for (std::size_t i = 0; i < v.size(); ++i) {
            for (std::size_t j = 0; j < v[i].size(); ++j) {
             ...
             // Pad with zeros if row was shorter (should not happen with writematrix)
    ...
    pudqlib::Graph load_graph_from_matlab_export(const std::string& directory_path) {
        ...
        // --- Load Edges (M x 2), auto-normalize to 0-based ---
        ...
        for (int i = 0; i < M; ++i) {
            ...
            if (idx == Indexing::One) { id1 -= 1; id2 -= 1; } // normalize 1->0
        ...
        // OPTIONAL: write a fixed copy for future runs (comment out if not wanted)
        // {
        //     std::ofstream ofs((dir / "edges_zero_based.txt").string());
        //     if (ofs) { for (auto &e : G.edges) ofs << e.first << " " << e.second << "\n"; }
        // }

        // --- Load Delta Poses PUDQ (M x 4) ---
        {
        ...
        // --- Load Delta Poses PUDQ True (M x 4) ---
        {
        ...
        // Final consistency checks (cheap insurance)
    ...
    // Helper for std::vector<double> (one value per line)
    ...
    // Helper for edges (writes M x 2 matrix, 0-indexed)
    ...
    // REMOVED local R_theta implementation.

    // MATLAB: reorient_graph
    ...
    void calculate_RPE_Euclidean(const pudqlib::Graph& G) {
        ...
        for (int ij = 0; ij < num_edges; ++ij) {
            ...
            // CHANGE: Use pudqlib::R_from_theta
    ...
    // ---------------------------------------------------------
    // Metric 3: ATE_G_pudq.m
    // ---------------------------------------------------------
    void calculate_ATE_PUDQ(const pudqlib::Graph& G) {
        double ATE = 0.0;
        int num_vertices = static_cast<int>(G.vertices.size());

        // Equivalent to MATLAB identity PUDQ
